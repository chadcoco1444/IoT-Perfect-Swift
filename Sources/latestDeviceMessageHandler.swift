...
public func latestDeviceMessage(request: HTTPRequest, response: HTTPResponse) {
        let mysql = MySQL()
		...
		let querySuccess = mysql.query(statement: "SELECT t.deviceID, t.sensorValue1, t.sensorValue2,t.sensorValue3,t.sensorValue4,t.sensorValue5,t.sensorValue6,t.sensorValue7,t.sensorValue8,t.ts from \(testTable) t inner join (     SELECT deviceID, MAX(ts) as MaxDate     FROM \(testTable) WHERE deviceID='\(deviceID)'    group by deviceID ) tm on t.deviceID = tm.deviceID and t.ts = tm.MaxDate ;")
		//let querySuccess = mysql.query(statement: "SELECT * FROM \(testTable) WHERE deviceID='\(deviceID)';")
